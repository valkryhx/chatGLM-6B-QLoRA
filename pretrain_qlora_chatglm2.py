...
def get_datset(data_path, tokenizer, global_args,max_samples=None):
    ...
    if max_samples is not None and max_samples > 0:
            ...
            data['train'] =  data['train'].shuffle(seed=global_args.seed).select(range(max_samples))
...
def group_texts(examples,block_size):
        ...
        #logger.error(f"examples.keys()={examples.keys()}") 这样会打印k和v
        logger.error(f"examples.keys()={list(examples.keys())}") #这样只打印keys
