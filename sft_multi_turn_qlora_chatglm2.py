...
def parse_args():
    ...
    parser.add_argument("--max_length",type=int,default=256,help="多个轮次对话的总文本的最大token数量  也就是history对应的多个对话的Q+A的整体token长度 与train_lora.py中的定义一致。")
...
def get_multi_turn_conversations_datset(data_path, tokenizer, max_samples=-1,global_args=None):
    ...
    if max_samples is not None and max_samples > 0:
            ...
            data['train'] =  data['train'].shuffle(seed=42).select(range(max_samples))
