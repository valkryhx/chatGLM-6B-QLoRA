...
class RewardModel_old(PreTrainedModel):
...
def preprocess_function_old(example,tokenizer):
    new_examples = {
        "input_ids_j": [],
        "attention_mask_j": [],
        "input_ids_k": [],
...
class RewardModel(nn.Module):
    ...
    def forward(self,
        ...
        if self.model_type == "chatglm":  ## chatglm的hidden_states需要特殊处理 其他模型也要注意 参考https://github.com/valkryhx/ChatGLM-LoRA-RLHF-PyTorch/blob/main/reward_model.py#L65C43-L65C43
            hidden_states = transformer_outputs[0]
...
def preprocess_function2(example,tokenizer):
    #
    # for question, response_j, response_k in zip(examples["question"], examples["response_j"], examples["response_k"]):
    #for question, response_j, response_k in zip(example["user_input"], example["completion_a"], example["completion_b"]):
    ...
    tokenized_k = tokenizer(
        ...
        # new_examples["input_ids_k"].append(tokenized_k["input_ids"])
        # new_examples["attention_mask_k"].append(tokenized_k["attention_mask"])
    return [ tokenized_j["input_ids"]  ,tokenized_j["attention_mask"] , tokenized_k["input_ids"] ,tokenized_k["attention_mask"]]
...
class RewardDataCollatorWithPadding2:
...
def train(global_args):
    ...
    model_config = AutoConfig.from_pretrained(global_args.model_name_or_path,trust_remote_code=True,)
    model_config.dropout = 0.0  # RM dropout=0
