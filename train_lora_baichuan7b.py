...
def train(global_args):
    ...
    model = AutoModelForCausalLM.from_pretrained(global_args.model_name_or_path,
                                          ...
                                          # low_cpu_mem_usage=True 是devicemap=auto时自动开启的  
                                          #https://huggingface.co/docs/transformers/main_classes/model#large-model-loading       
                                          #这一行注视掉empty_init=False,   # https://github.com/THUDM/ChatGLM-6B/issues/530
    ...
    logger.error("建议在这里打印一下peftmodel的某一层参数看看")
    ...
    data_collator = DataCollatorForLLM(pad_token_id=tokenizer.pad_token_id,
